...
class TextMelSpeakerAccentQwenDataset(torch.utils.data.Dataset):
    def __init__(self, filelist_path, cmudict_path, add_blank=True,
        ...
        # random.seed(random_seed)
        # random.shuffle(self.filelist)
        self.llm_embeds = torch.load('accent_embeddings2.pt')
        if train:
            self.lengths_dict = self.get_lengths()
            # a = self.write_lengths()
            self.lengths = [self.lengths_dict[key[0]] for key in self.filelist]
    ...
    def get_spk_emb(self, filepath):
        if 'WAV' in filepath:
            spk_emb_path = filepath.replace('WAV', 'facodec_spk')
        else:
    ...
    def get_triplet(self, line, whisper):
        ...
        return (text, mel, mel_whisper, speaker, accent, filepath, llm_embed, spk_emb)

        # return (text, mel, mel_whisper, speaker, accent, filepath)

    def get_mel(self, filepath, whisper_flag=False):
        audio, sr = ta.load(filepath)
        assert sr == self.sample_rate, filepath
        mel = mel_spectrogram_align(audio, self.n_fft, self.n_mels, self.sample_rate, self.hop_length,
                              self.win_length, self.f_min, self.f_max, center=False).squeeze()
        if whisper_flag:
            # print("whisper mel")
    ...
    def write_lengths(self):
        ...
        print(self.lengths_max)
        with open('lengths_acc.json', 'w', encoding='utf8') as output:
            json.dump(self.lengths, output, indent=4)
            
        return self.lengths
    
    def get_lengths(self):
        with open('lengths_acc.json', 'r', encoding='utf8') as input:
            self.lengths_dict = json.load(input)
        # self.lengths_max = 3000
        return self.lengths_dict
    ...
    def __getitem__(self, index):
        text, mel, mel_whisper, speaker, accent, filepath, llm_embed, spk_emb = self.get_triplet(self.filelist[index], self.whisper_flag)
        item = {'y': mel, 'y_prompt': mel_whisper, 'x': text, 'spk': speaker, 'acc': accent, 'filepath': filepath, 'llm_embed': llm_embed, 'spk_emb': spk_emb}
        # print(self.filelist[index])
        # print(item['y'].shape)
        return item
...
class TextMelSpeakerAccentQwenBatchCollate(object):
    def __call__(self, batch):
        ...
        for i, item in enumerate(batch):
            y_, x_, spk_, acc_, y_prompt_, llm_, spk_emb_ = item['y'], item['x'], item['spk'], item["acc"], item['y_prompt'], item['llm_embed'], item['spk_emb']
            # print(y_.shape, x_.shape, y_prompt_.shape, llm_.shape, spk_emb_.shape, item['filepath'])
            y_lengths.append(y_.shape[-1])
            x_lengths.append(x_.shape[-1])
            y[i, :, :y_.shape[-1]] = y_
            x[i, :x_.shape[-1]] = x_
        ...
        spk = torch.cat(spk, dim=0)
        acc = torch.cat(acc, dim=0)
        return {'x': x, 'x_lengths': x_lengths, 'y': y, 'y_lengths': y_lengths, 'y_prompt': y_prompt, 'spk': spk, 'acc': acc, 'filepath': item['filepath'], 'llm_embs': llm_embeds, 'spk_embs': spk_embs}
