...
class SharedEncoder(nn.Module):
    def __init__(self, n_mels: int, n_ctx: int, n_state: int, n_head: int, n_layer: int, n_accents: int = 6):
        ...
        self.vocab_project = nn.Linear(2 * n_state, 1927)
...
class Whisper(nn.Module):
    def __init__(self, dims: ModelDimensions, n_accents: int = 6,  n_speakers: int = 32):
        super().__init__()
        self.dims = dims
        ...
        self.acc_head = PredHead(vocab_size=n_accents, hidden_size=self.dims.n_audio_state, num_layers=2, feats_size=256)
        self.spk_head = PredHead(vocab_size=n_speakers, hidden_size=self.dims.n_audio_state, num_layers=2, feats_size=256)
