...
def load_models(acoustic_checkpoint_path=None, mel_output_dir=None):
    ...
    generator = GradTTSConformerGSTWhisper3Qwen2facodec3accrmllm(zh_dict.__len__() + 1, params.n_spks, params.spk_emb_dim,
                        ...
                        # acc_cln_layer=0, spk_cln_layer=5, spk_dec=False, acc_dec=False)  # E1
                        # acc_cln_layer=0, spk_cln_layer=5, spk_dec=True, acc_dec=True) # E3
                        acc_cln_layer=-1, spk_cln_layer=-1, spk_dec=True, acc_dec=True)
...
def prepare_data(llm_embeds, spk, acc, uid, accfilepath, spkfilepath):

    if acc == 'north':
        emb_list = llm_embeds['北方']
    elif acc == 'south':
        emb_list = llm_embeds['南方']
    elif acc == 'sichuan':
        print('use sichuan llm emb')
        emb_list = llm_embeds['四川']
    elif acc == 'sg':
        emb_list = llm_embeds["新加坡华语"]
    else:
        raise KeyError

    idx = random.randint(0, len(emb_list) - 1)
    llm_embed = emb_list[idx].cuda()
    ...
    return llm_embed, y, y_lengths, y_prompt_spk
...
accent_pool = {
    'sg': '/home/xintong/Speech-Backbones/Grad-TTS/accent_testsets/prompt_acc/sg/A0001_S001_0_G0002_segment_0134.wav',
    'SSB0623': '/data2/xintong/aishell3/test/wav_16k/SSB0623/SSB06230059.wav',
    'SSB0629': '/data2/xintong/aishell3/train/wav_16k/SSB0629/SSB06290387.wav',
    'SSB0863': '/data2/xintong/aishell3/test/wav_16k/SSB0863/SSB08630099.wav',
    'SSB0693': '/data2/xintong/aishell3/test/wav_16k/SSB0693/SSB06930020.wav',
...
def infer_text_to_mel_for_speaker(spk):
    """
    对一个speaker用TEXT_POOL里的所有句子合成
    spk: 说话人ID (SSBxxxx 或 Gxxxx)
    uid_list: 可选,手动指定wav uid,否则从TEXT_POOL里自动取
    """
    global generator, zh_dict, output_dir, fa_encoder, fa_decoder

    llm_embeds = torch.load('accent_embeddings.pt')
    ...
    for line in TEXT_POOL:
        ...
        for accfilepath in accfilepaths:
            ...
            # 准备 llm_embed, y, y_lengths, y_prompt_spk
            if 'sg' in accfilepath:
                acc = 'sg'
            else:
                if spk in spk2acc:
                    acc = spk2acc[spk]
                    ...
                    acc = 'sichuan'
            print(f"Using accent {acc} from {accfilepath}, acc emb from {acc_embs_path}")
            llm_embed, y, y_lengths, y_prompt_spk = prepare_data(
                llm_embeds, spk, acc, uid, accfilepath, spkfilepath
            )

            # 推理 mel
            with torch.no_grad():
                y_enc, y_dec, attn = generator.prompt(
                    x, x_lengths, y, y_lengths, y_prompt_spk,
                    llm_embed, spk_embs, acc_embs,
                    n_timesteps=10, temperature=1.5, stoc=False,
                    spk=spk, acc=acc, length_scale=0.91
                )
...
if __name__ == "__main__":
    ...
    acoustic_checkpoint_path = 'logs/joycent_e2/grad_272.pt'
    ...
    model = acoustic_checkpoint_path.split('/')[1] + 'e' + acoustic_checkpoint_path.split('/')[2].split('_')[1][:-3]

    root = '/data2/xintong/accent_testsets/evaluation'
