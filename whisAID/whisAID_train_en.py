...
class WhisperModelModule(LightningModule):
    ...
    def training_step(self, batch, batch_id):
        ...
        # _, pred_spk_D = self.model.spk_grl(feats_acc_mean_D.squeeze(1))
        # loss_spk_grl_D = F.cross_entropy(pred_spk_D.squeeze(1), spks)

        # self.optimizer_spk.zero_grad()
        # self.manual_backward(loss_spk_grl_D)
        # self.optimizer_spk.step()

        # generator
        # _, pred_spk_adv = self.model.spk_grl(feats_acc_mean.squeeze(1))
        # loss_spk_grl_adv = F.cross_entropy(pred_spk_adv.squeeze(1), spks)
        loss = accent_loss
        # loss = loss_acc + loss_spk + loss_spk_grl_adv
    ...
    def configure_optimizers(self):
        ...
        return [self.optimizer], [{"scheduler": scheduler, "interval": "step", "frequency": 1}]
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--train-path",
        type=str,
        default="resources/whisAID/CommonAccent/train.csv",
        help="Batch size for training",
    )
    parser.add_argument(
        "--val-path",
