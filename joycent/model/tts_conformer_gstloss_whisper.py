...
from joycent.model.module_gstloss_grl_whisper import GSTWhisper
from joycent.model.classifier import GradientReversalFunction
from torch.nn import functional as F
from copy import deepcopy

class GradTTSConformerGSTWhisper(BaseModule):
    def __init__(self, n_vocab, n_spks, spk_emb_dim, n_enc_channels, filter_channels, filter_channels_dp, 
        ...
        super(GradTTSConformerGSTWhisper, self).__init__()
    ...
    def forward(self, x, x_lengths, y, y_lengths_ref, n_timesteps, cond=None, temperature=1.0, stoc=False, spk=None, acc=None, length_scale=1.0):
        ...
        if self.gst:
            logits_acc, logits_spk = self.gst(y, y_lengths_ref)
    ...
    def compute_loss(self, x, x_lengths, y_prompt, y, y_lengths, cond=None, spk=None, acc=None, out_size=None):
        ...
        # y_cut = y
        if self.gst:
            logits_acc, logits_spk = self.gst(y_prompt, y_lengths)
            logits_acc_mean = logits_acc.mean(dim=1)
            logits_spk_mean = logits_spk.mean(dim=1)
