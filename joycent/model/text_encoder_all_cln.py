...
from joycent.model.utils import sequence_mask, convert_pad_shape
from joycent.model.conformer.encoder_rand_cln import ConformerBlock
...
class Encoder(BaseModule):
    def __init__(self, hidden_channels, filter_channels, n_heads, n_layers, 
                 kernel_size=1, p_dropout=0.0, window_size=None, cln=False, **kwargs):
        ...
        for i in range(self.n_layers):
            ...
            if self.cln and i == 0:
                self.norm_layers_2.append(Condional_LayerNorm(hidden_channels))
            else:
                self.norm_layers_2.append(LayerNorm(hidden_channels))

    def forward(self, x, x_mask, cond=None):
        ...
        for i in range(self.n_layers):
            ...
            y = self.drop(y)
            if self.cln and i == 0:
                # print((x+y).shape, cond.shape)
                
                x = self.norm_layers_2[i]((x + y).transpose(1,2), cond)
                x = x.transpose(1,2)
            else:
...
class ConformerEncoder(BaseModule):
    def __init__(self, hidden_channels, filter_channels, n_heads, n_layers, 
                 kernel_size=1, p_dropout=0.0, window_size=None, cln=False, acc_cln_layer=3, spk_cln_layer=5):
        ...
        self.cln = cln
        self.acc_cln_layer = acc_cln_layer
        self.spk_cln_layer = spk_cln_layer
        
        acc_cln_layer = torch.randint(1, 6, (1,)).item()
        for i in range(self.n_layers):
            if self.cln and (i == acc_cln_layer or i == spk_cln_layer):
    ...
    def forward(self, x, x_mask, acc_cond=False, spk_cond=False):
        # attn_mask = x_mask.unsqueeze(2) * x_mask.unsqueeze(-1)
        # print(x.shape, x_mask.shape)
        # chosen_layer = random.randint(0, len(self.layers) - 2)
        # print("Chosen layer for acc cond: {}".format(chosen_layer))
        # for i, layer in enumerate(self.layers):
        #     x = layer(x)
        #     if i == chosen_layer:  # 只在这层加 CLN
        #         x = self.clns[i](x, cond)
        # return x
        ...
        # for layer in self.layers:
            ...
            if i == 4:
            # if i == 5:
                # print("Using acc cond at layer {}".format(i))
                x = layer(x, cond=acc_cond)
            elif i == len(self.layers) - 1:
                # print("Using spk cond at layer {}".format(i))
                x = layer(x, cond=spk_cond)
            else:
                x = layer(x, cond=None)
            # else:
            #     # print("No cond at layer {}".format(i))
            #     x = layer(x, cond=None) 
            x = x.transpose(1, 2)
        x = x * x_mask
        return x
...
class TextConformerEncoder(BaseModule):
    def __init__(self, n_vocab, n_feats, n_channels, filter_channels, 
                 filter_channels_dp, n_heads, n_layers, kernel_size, 
                 p_dropout, window_size=None, spk_emb_dim=64, n_spks=1, cln=False, acc_cln_layer=3, spk_cln_layer=5):
        ...
        self.encoder = ConformerEncoder(n_channels + (spk_emb_dim if n_spks > 1 else 0), filter_channels, n_heads, n_layers, 
                               kernel_size, p_dropout, window_size=window_size, cln=cln, acc_cln_layer=acc_cln_layer, spk_cln_layer=spk_cln_layer)

        self.proj_m = torch.nn.Conv1d(n_channels + (spk_emb_dim if n_spks > 1 else 0), n_feats, 1)
        self.proj_w = DurationPredictor(n_channels + (spk_emb_dim if n_spks > 1 else 0), filter_channels_dp, 
                                        kernel_size, p_dropout)

    def forward(self, x, x_lengths, acc_cond=None, spk_cond=None):
        x = self.emb(x) * math.sqrt(self.n_channels)
        x = torch.transpose(x, 1, -1)
        x_mask = torch.unsqueeze(sequence_mask(x_lengths, x.size(2)), 1).to(x.dtype)
        ...
        x = self.encoder(x, x_mask, acc_cond=acc_cond, spk_cond=spk_cond)
        mu = self.proj_m(x) * x_mask
