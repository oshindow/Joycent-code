...
class GradLogPEstimator2d(BaseModule):
    def __init__(self, dim, dim_mults=(1, 2, 4), groups=8,
        ...
        print(self.n_spks, self.n_accents)
        dims = [2 + (1 if self.n_spks > 1 else 0) + (1 if self.n_accents > 1 else 0), *map(lambda m: dim * m, dim_mults)]
    ...
    def forward(self, x, mask, mu, t, spk=None, acc=None):
        ...
        if not isinstance(spk, type(None)):
            # print("spk is not None, concate")
            s = self.spk_mlp(spk)
        
        if not isinstance(acc, type(None)):
            # print("acc is not None, concate")
            a = self.acc_mlp(acc)
